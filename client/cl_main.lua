...
RegisterNetEvent('dispatch:manageNotifs', function(sentSetting)
    ...
    if wantedSetting == "on" then
        ...
        TriggerEvent("Notify", "sucesso", "Disabled!")
    elseif wantedSetting == "off" then
        disableNotis = true
        disableNotifSounds = true
        TriggerEvent("Notify", "sucesso", "Enable!")
    elseif wantedSetting == "mute" then
        disableNotis = false
        disableNotifSounds = true
        TriggerEvent("Notify", "sucesso", "Muted!")
    else
		TriggerEvent("Notify", "sucesso", 'Please choose to have dispatch as "on", "off" or "mute".')
    end
end)
...
RegisterNetEvent("nc-dispatch:client:clearAllBlips", function()
	...
	TriggerEvent("Notify", "sucesso", 'All dispatch blips cleared')
end)
